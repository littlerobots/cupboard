...
/* This variant handles the use of Annotations that allows one to decouple a field
 * name from a column name, by specifying the column name in an Annotation.
...
public class AnnotatedConverter<T> implements Converter<T> {
	/*
	 * 'name' will be taken from the Annotation if present,
	 * otherwise from the field name as per DefaultConverter.
	 */
    ...
    private static HashMap<Class<?>, TypeConverter<?>> sTypeConverters = new HashMap<Class<?>, AnnotatedConverter.TypeConverter<?>>(10);
    ...
    public AnnotatedConverter(Class<T> clz, Map<Class<?>, ConverterHolder<?>> entities) {
        Field[] fields = clz.getDeclaredFields();
        mColumns = new ArrayList<Converter.Column>(fields.length);
        this.mClass = clz;
        List<Property> properties = new ArrayList<AnnotatedConverter.Property>();
        for (Field field : fields) {
            if (!Modifier.isTransient(field.getModifiers()) && !Modifier.isFinal(field.getModifiers()) && !Modifier.isStatic(field.getModifiers())) {
                ...
                /*
                 * note: getColumn method will return either the Annotation value
                 * or the field name.
                 */
    ...
    /*
     * Tweaked version, specifically for AnnotatedConverter, that
     * returns the Annotated columnName if present, otherwise the field
     * name as per DefaultConverter.
     */
    protected String getColumn(Field field) {
    	if(field.isAnnotationPresent(CupboardColumn.class)) {
    		return field.getAnnotation(CupboardColumn.class).columnName();
    	}
